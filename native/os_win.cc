...
OSWindow OSWindow::FromJsValue(const Napi::Value jsval)
{
	...
	if (!lossless) {
		throw Napi::RangeError::New(jsval.Env(), "Invalid handle");
	}
...
void OSSetWindowParent(OSWindow wnd, OSWindow parent)
{
	...
	if (parent.handle != 0) {
		SetWindowPos(wnd.handle, parent.handle, 0, 0, 0, 0,
			SWP_ASYNCWINDOWPOS | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE);
		SetWindowPos(parent.handle, wnd.handle, 0, 0, 0, 0,
			SWP_ASYNCWINDOWPOS | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE);
	}
...
bool IsRsWindow(HWND hwnd)
{
	if (hwnd != 0) {
		...
		if (GetClassNameW(hwnd, wname, name_len) != 0) {
			if (WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)wname, -1, (LPSTR)&name,
...
void OSCaptureOpenGLMulti(OSWindow wnd, vector<CaptureRect> rects, Napi::Env env)
{
	...
	if (!pixeldata) {
		...
		throw Napi::Error::New(
			env, string() + "Failed to capture, native error: " + errtext);
	}
...
void OSCaptureMulti(OSWindow wnd, CaptureMode mode, vector<CaptureRect> rects, Napi::Env env)
{
	...
	case CaptureMode::Desktop: {
		...
		for (auto &capt : mapped) {
			OSCaptureDesktop(capt.data, capt.size, capt.rect.x + offset.x,
				capt.rect.y + offset.y, capt.rect.width, capt.rect.height);
		}
		break;
	}
	case CaptureMode::Window:
		for (auto const &capt : rects) {
			OSCaptureWindow(capt.data, capt.size, wnd, capt.rect.x, capt.rect.y,
				capt.rect.width, capt.rect.height);
		}
		break;
...
void HookProc(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild,
...
struct WindowsEventHook {
	...
	WindowsEventHook(HWND hwnd, WindowsEventGroup group) : hwnd(hwnd), group(group)
	{
		...
		case WindowsEventGroup::Object:
			eventhandle = SetWinEventHook(0x8000, 0x800B, 0, HookProc, pid, 0,
				WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS);
			break;
		case WindowsEventGroup::System:
			eventhandle = SetWinEventHook(0x0000, 0x0030, 0, HookProc, pid, 0,
				WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS);
			break;
		default:
			assert(false);
...
template <typename F, typename COND> void iterateHandlers(COND cond, F tracker)
{
	...
	DWORD idEventThread, DWORD dwmsEventTime)
{
	...
	case EVENT_OBJECT_DESTROY:
		iterateHandlers(
			[hwnd](const TrackedEvent &h) {
				return hwnd == h.wnd.handle && h.type == WindowEventType::Close;
			},
	...
	case EVENT_SYSTEM_CAPTURESTART: {
		...
		iterateHandlers(
			[windowhwnd](const TrackedEvent &h) {
				return windowhwnd == h.wnd.handle &&
				       h.type == WindowEventType::Click;
			},
			[](const std::shared_ptr<Napi::FunctionReference> &h) {
				auto env = h->Env();
				Napi::HandleScope scope(env);
	...
	case EVENT_OBJECT_LOCATIONCHANGE: {
		...
		const char *phase = (event == EVENT_SYSTEM_MOVESIZEEND   ? "end" :
				     event == EVENT_SYSTEM_MOVESIZESTART ? "start" :
									   "moving");
		iterateHandlers(
			[hwnd](const TrackedEvent &h) {
				return hwnd == h.wnd.handle && h.type == WindowEventType::Move;
			},
	...
	case EVENT_OBJECT_CREATE: {
		if (IsRsWindow(hwnd)) {
			iterateHandlers(
				[hwnd](const TrackedEvent &h) {
					return (h.wnd.handle == 0 || hwnd == h.wnd.handle) &&
					       h.type == WindowEventType::Show;
				},
				[hwnd, event](const std::shared_ptr<Napi::FunctionReference> &h) {
					...
					try {
						h->MakeCallback(env.Global(),
							{ Napi::BigInt::New(env, (uint64_t)hwnd),
...
TrackedEvent::TrackedEvent(OSWindow wnd, WindowEventType type, Napi::Function cb)
{
	...
	case WindowEventType::Click:
		this->hooks = { WindowsEventHook::GetHook(wnd.handle, WindowsEventGroup::System) };
		break;
	case WindowEventType::Move:
		this->hooks = { WindowsEventHook::GetHook(wnd.handle, WindowsEventGroup::Object),
