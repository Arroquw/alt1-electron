...
const NSMutableDictionary<NSNumber *, NSMutableArray<NSView *> *> *trackedViews =
...
static inline bool ax_privilege()
{
	...
	CFDictionaryRef options = CFDictionaryCreate(nullptr, keys, values, array_count(keys),
...
static void ax_callback(
	AXObserverRef observer, AXUIElementRef element, CFStringRef notification, void *refcon)
{
	pid_t pid;
...
+ (void)handleRsDidLaunch:(pid_t)pid
{
	if (observers.find(pid) == observers.end()) {
		...
		if ([AOUtil updateNotifications:true
				    forObserver:obs
				     withAppRef:appRef
			       withReferenceObj:nullptr
			      withNotifications:kAXApplicationShownNotification,
			...
			tryGetWindow = [^{
				...
				if (windowId != kCGNullWindowID) {
					...
					if (trackedWindows[pidNum] == nil) {
						...
						NSLog(@"mac: Already tracking window for pid %d",
							pid);
					}
				} else {
					NSLog(@"Failed to get RS window, returned null for pid %d",
						pid);
				}
			} copy];
			tryGetWindow();
...
+ (void)handleRsDidTerminate:(pid_t)pid
{
	if (observers.find(pid) != observers.end()) {
		...
		if ([AOUtil updateNotifications:false
				    forObserver:obs
				     withAppRef:appRef
			       withReferenceObj:NULL
			      withNotifications:kAXApplicationShownNotification,
			    kAXApplicationHiddenNotification, kAXApplicationActivatedNotification,
			    kAXApplicationDeactivatedNotification,
			    kAXWindowMiniaturizedNotification, kAXWindowDeminiaturizedNotification,
...
+ (void)handleAXMove:(pid_t)pid
{
	...
	JSRectangle bounds = JSRectangle(static_cast<int>(winRect.origin.x),
		...
		static_cast<int>(winRect.size.height));
	[AOTrackedEvent
		IterateEvents:^BOOL(AOTrackedEvent *e) {
...
+ (BOOL)shouldBeOnTop
{
	return _frontPid == [[NSRunningApplication currentApplication] processIdentifier] ||
	       _frontPid == _rsPid;
}
...
+ (BOOL)isFullScreen:(CGRect)bounds
{
	...
	free(displays);
	return (bounds.origin.x == screenBounds.origin.x &&
		bounds.origin.y == screenBounds.origin.y &&
		bounds.size.width == screenBounds.size.width &&
		bounds.size.height == screenBounds.size.height);
}
...
+ (pid_t)focusedPid
{
	...
	if (kAXErrorSuccess == AXUIElementCopyAttributeValue(frontMostSys,
				       kAXFocusedApplicationAttribute,
				       (CFTypeRef *)&frontMostApp)) {
		pid_t pid;
...
+ (CGWindowID)appMainWindow:(pid_t)pid
{
	...
	// Copy window attributes
	AXError err = AXUIElementCopyAttributeValue(
		frontMostApp, kAXMainWindowAttribute, (CFTypeRef *)&frontMostWindow);
	if (err != kAXErrorSuccess) {
...
+ (NSString *)appTitle:(pid_t)pid
{
	...
	err = AXUIElementCopyAttributeValue(
		frontMostWindow, kAXTitleAttribute, (CFTypeRef *)&frontMostWindowTitle);
	if (err != kAXErrorSuccess) {
		NSLog(@"no window size %d", err);
		if (frontMostApp != nil) {
			CFRelease(frontMostApp);
...
+ (void)capture:(OSWindow)wnd withRects:(vector<CaptureRect>)rects
{
	...
	for (auto &rect : rects) {
		CGRect cropRect =
			CGRectMake(rect.rect.x, rect.rect.y, rect.rect.width, rect.rect.height);
...
+ (BOOL)drawImage:(CGImageRef)image ontoBuffer:(void *)theData withScale:(CGFloat)scale
{
	...
	CGColorSpaceRef colorspace = CGColorSpaceCreateWithName(kCGColorSpaceSRGB);
	CGContextRef context = CGBitmapContextCreate(
		theData, width, height, 8, width * 4, colorspace, kCGImageAlphaPremultipliedLast);
	CGColorSpaceRelease(colorspace);
	if (context == NULL)
		return false;
	CGContextSetInterpolationQuality(context, kCGInterpolationNone);
...
+ (void)captureImageFile:(CGImageRef)imageRef withFilename:(NSString *)filename
{
	...
	CGImageDestinationRef destination =
		CGImageDestinationCreateWithURL(fileURL, CFSTR("public.png"), 1, nullptr);
...
+ (void)interceptDelegate:(NSWindow *)window
{
	if ([window delegate] == nil ||
		[[[window delegate] class] isEqualTo:[AONSWindowDelegate class]]) {
		return;
	}
