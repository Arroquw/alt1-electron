...
{
XShmCapture::XShmCapture(xcb_connection_t *c, xcb_drawable_t d)
	: connection(c), drawable(d), geometry(NULL, &free)
{
	xcb_get_geometry_cookie_t cookie = xcb_get_geometry_unchecked(c, d);
	geometry = std::unique_ptr<xcb_get_geometry_reply_t, decltype(&free)>{
		xcb_get_geometry_reply(c, cookie, NULL), &free
	};
	if (!geometry) {
	...
	xcb_shm_get_image_cookie_t imageCookie = xcb_shm_get_image(c, d, 0, 0, geometry->width,
		geometry->height, 0xFFFFFF, XCB_IMAGE_FORMAT_Z_PIXMAP, this->shmSeg, 0);
	std::unique_ptr<xcb_shm_get_image_reply_t, decltype(&free)> getImageReply{
		xcb_shm_get_image_reply(c, imageCookie, NULL), &free
	};
	if (!getImageReply) {
...
void XShmCapture::copy(char *target, size_t maxLength, int x, int y, int w, int h)
{
	...
	for (int row = y; row < y + h; row++) {
		for (int col = x; col < x + w; col++) {
			if (col >= 0 && row >= 0 && col < this->geometry->width &&
				row < this->geometry->height) {
